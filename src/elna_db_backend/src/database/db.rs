...
use oasysdb::{
    collection::{Record, SearchResult},
    vector::Vector,
};
...
impl Database {
    ...
    pub fn create_collection(&mut self, name: String, dimension: usize) -> Result<(), Error> {
        ...
        let collection: Collection = Collection::new(dimension);
    ...
    pub fn insert_into_collection(
        ...
        let all_same_length = keys
            .iter()
            .all(|inner| inner.vector.len() == collection.dimension);
        ...
        collection
            .append(&keys)
            .map_err(|_| Error::UniqueViolation)?;
    ...
    pub fn query(
        ...
        let result = collection
            .query(&v, limit)
            .map_err(|_| String::from("query failed"))?;
